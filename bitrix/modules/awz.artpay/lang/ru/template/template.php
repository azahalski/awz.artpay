<?php
$MESS["AWZ_ARTPAY_HANDLER_TMPL_DESCRIPTION"] = "Услугу предоставляет сервис онлайн-платежей <b>&laquo;ArtPay&raquo;</b>.<br /><br />Сумма к оплате по счету: ";
$MESS['AWZ_ARTPAY_HANDLER_TMPL_ERIP_INSTRUCTION'] = '
Если Вы осуществляете платеж в кассе банка, пожалуйста, сообщите кассиру о необходимости проведения платежа через ЕРИП.</br>
<br>
Для проведения платежа необходимо найти магазин в дереве ЕРИП по коду услуги <strong>#ERIP_SERVICE_CODE#</strong> или воспользоваться инструкцией:<br>
<ol>
<li>Выбрать пункт ЕРИП</li>
<li>Выбрать последовательно пункты: <i>#INSTRUCTION#</i></li>
<li>Ввести номер <strong>#ACCOUNT_NUMBER#</strong></li>
<li>Проверить корректность информации</li>
<li>Совершить платеж</li>
</ol>';
$MESS['AWZ_ARTPAY_HANDLER_TMPL_EPOS_INSTRUCTION'] = '
Если Вы осуществляете платеж в кассе банка, пожалуйста, сообщите кассиру о необходимости проведения платежа через E-POS.</br>
<br>
Для проведения платежа авторизуйтесь в интернет-банкинге (приложении или web-версии):<br>
<ol>
<li>Выберите пункт ЕРИП</li>
<li>Выберите "Сервис E-POS" в дереве услуг ЕРИП</li>
<li>Введите код заказа E-POS: <strong>#ACCOUNT_NUMBER#</strong></li>
<li>Проверьте корректность информации</li>
<li>Совершите платеж</li>
</ol>';
$MESS['AWZ_ARTPAY_HANDLER_TMPL_EPOS_SHOW_LINK_INSTRUCTION'] = 'Чтобы установить приложение для оплаты перейдите по <a target="_blank" href="#LINK#">ссылке</a>.';
$MESS['AWZ_ARTPAY_HANDLER_TMPL_EPOS_SHOW_QR_INSTRUCTION'] = 'Если банковское приложение поддерживает QR коды, отсканируйте код ниже для оплаты: <br><img src="#IMG#" alt="" title=""/>';